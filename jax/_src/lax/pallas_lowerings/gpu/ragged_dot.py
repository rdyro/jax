...
def _pallas_ragged_dot_general_impl(
  ...
  if (not config.jax_ragged_dot_use_gpu_pallas_triton_lowering.value
      or not _backend_supports_triton()):
    # Allow fallback to the base jax._src.lax lowering if the config is not set.
    return lax._ragged_dot_general_impl(
        lhs, rhs, group_sizes, ragged_dot_dimension_numbers,
...
lax.mlir.register_lowering(
    lax.ragged_dot_general_p,
    lax.mlir.lower_fun(_pallas_ragged_dot_general_impl, multiple_results=False),
    platform="gpu",
)
