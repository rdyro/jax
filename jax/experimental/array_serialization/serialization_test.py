...
from jax.experimental.array_serialization import pytree_serialization_utils
...
class UserAPITest(UserAPITestCase):
  ...
  def test_custom_types(self, use_node, use_dataclass, use_static, save_pickle,
    ...
    if save_pickle:
      pytree_serialization.save(tree_to_save, self.path, pickle_module=pickle)
    else:
      with self.assertRaises(ValueError):
        pytree_serialization.save(tree_to_save, self.path, pickle_module=None)
      return

    if load_pickle:
      tree2 = pytree_serialization.load(self.path, pickle_module=pickle)
    else:
      with self.assertRaises(ValueError):
        _ = pytree_serialization.load(self.path, pickle_module=None)
      return
  ...
  def test_best_effort(self, register):
    ...
    with self.assertRaises(ValueError):
      _ = pytree_serialization.load(self.path, pickle_module=None)
    _ = pytree_serialization.load(self.path, pickle_module=None, best_effort=True)
  ...
  def test_flax_frozen_dict(self):
    ...
    extended_node_types = pytree_serialization_utils._EXTENDED_NODE_TYPES_MAP
    with absltest.mock.patch(pytree_serialization_utils.__name__
                             + '._EXTENDED_NODE_TYPES_MAP', {
        **extended_node_types, **{'flax.core.frozen_dict.FrozenDict': (
            'nonexisting_module3434.FrozenDict')}}):
      with self.assertRaises(ImportError):
        pytree_serialization.load(self.path)
      pytree_serialization.load(self.path, best_effort=True)
    pytree_serialization.load(self.path)
