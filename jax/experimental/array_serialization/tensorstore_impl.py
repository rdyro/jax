...
def _maximum(x: Sequence[int], default: int = 1) -> list[int]:
  ...
  return list(max(z, default) for z in x)
...
def _compute_chunk_shape(
    arr: jax.Array, file_size_target: int = _FILE_SIZE_TARGET) -> list[int]:
  ...
  while chunk_size > 1.1 * file_size_target:  # 10% buffer
    chosen_axis_idx, chosen_divisor = None, None
    for axis_idx in range(len(local_shape)):
      if len(axis_prime_factors[axis_idx]) == 0:
        continue
    ...
    if chosen_axis_idx is None:
      break
    axis_prime_factors[chosen_axis_idx].pop(0)
...
def get_tensorstore_metadata(arr, is_remote: bool = False,
  ...
  if driver == TS_ARRAY_DRIVER:
    codecs = ([{"name": "crc32c"}, {"name": "zstd"}]
              if is_remote else [{"name": "crc32c"}])
    return {
        'codecs': codecs,
        'shape': arr.shape,
        'data_type': jnp.dtype(arr.dtype).name,
        'chunk_grid': {
          'name': 'regular',
          'configuration': {'chunk_shape': _compute_chunk_shape(
              arr, file_size_target=file_size_target)}
        }
    }
  elif driver == "zarr":  # in zarr dtype goes in the base spec
    local_shape = arr.addressable_data(0).shape
    return {'compressor': {'id': 'zstd'}, 'shape': arr.shape,
...
def get_tensorstore_spec(
  ...
  if arr is not None:
    spec["metadata"] = get_tensorstore_metadata(arr, driver=spec["driver"])
  return spec
...
async def combine_kvstores(combined_kvstore: dict[str, Any],
  ...
  combined = ts.KvStore.open(combined_kvstore, context=context)
  kvstores = [ts.KvStore.open(kvstore, context=context) for kvstore in kvstores]
  combined, kvstores = await asyncio.gather(combined, asyncio.gather(*kvstores))
  tx = ts.Transaction()
...
async def async_deserialize(
    ...
    dtype=None,
    byte_limiter: asyncio_utils.LimitInFlightBytes | None = None,
    context=TS_CONTEXT,
    chunk_layout=TS_CHUNK_LAYOUT,
...
def run_deserialization(shardings: Sequence[jax.sharding.Sharding | Layout],
  ...
  async def _run_deserializer():
    ...
    byte_limiter = asyncio_utils.LimitInFlightBytes(concurrent_bytes)
